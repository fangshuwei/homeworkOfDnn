...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        # for SGD
        '''
        np_id = np.arange(0,60000,1)
        np_id = random.shuffle(np_id)
        x_new = np.zeros_like(x_train_norm)
        y_new = np.zeros_like(y_train_onehot)
        for num in range(0,60000):
            x_new[num] = x_train_norm[num]
            y_new[num] = y_train_onehot[num]
        _, c = sess.run([optimizer, cost], feed_dict={x: x_new, y: y_new})
        '''
        r = print(random.randint(0, 60000))
        _, c = sess.run([optimizer, cost], feed_dict={x: x_train_norm[0][r], y: y_train_onehot[0][r]})
